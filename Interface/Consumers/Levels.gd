...
func createbutton(level: int):
	...
	b.set_position(Vector2(viewportWidth/3.5 + (_BUTTON_GAP + _BUTTON_SIZE) * ((level - 1) % 3) ,viewportWidth/10 + (_BUTTON_SIZE + _BUTTON_GAP) *((level -1) / 3)))
...
func createStarCounter(button: Button, level: int):
	...
	sc.set_position(Vector2(viewportWidth/3.5 + (_BUTTON_GAP + _BUTTON_SIZE) * ((level - 1) % 3) , (viewportWidth/10) +(_BUTTON_SIZE/1.8) + (_BUTTON_SIZE + _BUTTON_GAP) *((level -1) / 3)))
